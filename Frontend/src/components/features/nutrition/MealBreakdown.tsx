...
export const MealBreakdown = ({ meal, mealName }: MealBreakdownProps) => {
  return (
    <Card className="bg-card border border-border rounded-2xl p-6 shadow-md">
      {/* Header with meal name and macro info */}
      <div className="flex items-center justify-between mb-4 flex-wrap gap-4">
        <TypographyH4 variant="bold">{mealName}</TypographyH4>
        <div className="flex items-center gap-4 text-muted-foreground text-sm font-normal">
          <span className="text-foreground font-semibold">{meal.calories.toFixed(1)} kcal</span>
      ...
      <div className="space-y-3">
        {meal.dishes.map((dish) => (
          <div key={dish.dishId} className="flex items-start gap-3">
            <Disc className="w-3 h-3 text-primary mt-1.5 shrink-0" fill="currentColor" />
            <div className="flex-1 flex items-start justify-between gap-4 flex-wrap">
              <span className="text-foreground font-normal">
                {dish.name} ({dish.quantity} phần)
              </span>
              <span className="text-muted-foreground text-md font-normal whitespace-nowrap">
