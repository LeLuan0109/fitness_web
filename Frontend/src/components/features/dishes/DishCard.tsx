...
export const DishCard: React.FC<{ dish: Dish; onClick?: () => void }> = ({ dish, onClick }) => {
  ...
  return (
    <Card
      className="group gap-0 overflow-hidden rounded-2xl border-sand/60 bg-card py-0 text-card-foreground shadow-sm shadow-earth/5 transition-all duration-300 hover:-translate-y-1 hover:border-clay/40 hover:shadow-xl hover:shadow-earth/10 cursor-pointer"
      onClick={onClick}
    >
